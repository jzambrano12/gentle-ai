...
| Substantial feature | Initiate SDD automatically: run `sdd-explore` then `sdd-propose`, delegate phases (async). If the user explicitly asks for SDD ("hazlo con sdd", "use sdd"), start immediately |
...
These commands are for YOU (the orchestrator) to use internally. Users do NOT need to know or type these — you invoke them organically based on the task size.

#### Phase skills (invoke via sub-agents)
- `sdd-init` -> Initialize SDD context. Detects stack, bootstraps persistence
- `sdd-explore <topic>` -> Investigate an idea. Reads codebase, compares approaches
- `sdd-propose <change>` -> Create a change proposal with intent, scope, approach
- `sdd-spec <change>` -> Write specifications with requirements and scenarios
- `sdd-design <change>` -> Create technical design with architecture decisions
- `sdd-tasks <change>` -> Break down into implementation task checklist
...
#### Orchestration flows (you compose these from the skills above)
- **New change**: run `sdd-explore` then `sdd-propose`
- **Continue**: create next missing artifact in the dependency chain
- **Fast-forward**: run `sdd-propose` → `sdd-spec` → `sdd-design` → `sdd-tasks`
